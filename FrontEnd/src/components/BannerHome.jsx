            ...
            bannerData.map((data,index)=>{
                return(
                    <div key={data.id+"bannerHome"+index} className='min-w-full min-h-[450px] lg:min-h-full overflow-hidden relative group transition-all' style={{transform:`translateX(-${currentImage*100}%)`}} >
                            ...
                            <div className='px-4 absolute top-0 w-full h-full hidden items-center justify-between group-hover:lg:flex'>
                              <button onClick={handlePrev} className='p-1 rounded-full text-black text-xl bg-white z-10'>
                              <FaAngleLeft />
                              </button>
                              <button onClick={handleNext} className='p-1 rounded-full text-black text-xl bg-white z-10'>
                              <FaAngleRight />
